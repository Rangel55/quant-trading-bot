...
function analyze(candles, orderBook) {
  ...
  var momentum = 'NEUTRAL';
    if (rsiVal && rsiVal > 50 && rsiVal < cfg.RSI_OVERBOUGHT) momentum = 'BULLISH';
    if (rsiVal && rsiVal < 50 && rsiVal > cfg.RSI_OVERSOLD) momentum = 'BEARISH';
  ...
  // Contagem de sinais
  const bullCount = (maTrendSignal === 'BULL' ? 1 : 0) + (momentum === 'BULLISH' ? 1 : 0) + (swing === 'UPTREND' ? 1 : 0) + (bookBias === 'BUY_PRESSURE' ? 1 : 0);
    const bearCount = (maTrendSignal === 'BEAR' ? 1 : 0) + (momentum === 'BEARISH' ? 1 : 0) + (swing === 'DOWNTREND' ? 1 : 0) + (bookBias === 'SELL_PRESSURE' ? 1 : 0);

  // DECISAO: minimo 2/4 sinais alinhados (era 3/4)
  // Volatilidade nao bloqueia mais a decisao
  var decision = 'ESPERAR';
    if (bullCount >= 2) decision = 'COMPRAR';
    if (bearCount >= 2) decision = 'VENDER';
    // Se ambos empatarem, usa RSI como desempate
  if (bullCount === bearCount && bullCount >= 2) {
        decision = (rsiVal && rsiVal > 50) ? 'COMPRAR' : 'VENDER';
  }

  return {
        price, maFast, maSlow, maTrend, rsiVal, atrVal,
        volatility, isVolatile, maTrendSignal, momentum,
        swing, bookBias, bullCount, bearCount, decision,
  };
}
