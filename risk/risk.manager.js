...
function resetDailyCounter() {
      ...
      if (lastTradeDate !== today) { dailyTradeCount = 0; lastTradeDate = today; }
}
function canTrade() {
      resetDailyCounter();
      if (dailyTradeCount >= cfg.MAX_DAILY_TRADES) { console.log('Limite diario atingido.'); return false; }
      return true;
}
...
function calculateLevels(price, side) {
      if (side === 'BUY') {
              return { stopLoss: price * (1 - cfg.STOP_LOSS_PERCENT), takeProfit: price * (1 + cfg.TAKE_PROFIT_PERCENT) };
      }
      return { stopLoss: price * (1 + cfg.STOP_LOSS_PERCENT), takeProfit: price * (1 - cfg.TAKE_PROFIT_PERCENT) };
}
function finalValidation(analysis) {
      var issues = [];
      if (!analysis.isVolatile) issues.push('Volatilidade insuficiente');
      if (analysis.decision === 'ESPERAR') issues.push('Sinais insuficientes');
      if (analysis.rsiVal > 85) issues.push('RSI sobrecomprado (' + analysis.rsiVal.toFixed(1) + ')');
      if (analysis.rsiVal < 15) issues.push('RSI sobrevendido (' + analysis.rsiVal.toFixed(1) + ')');
