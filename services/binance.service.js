...
// CORRIGIDO: nao relanca erro - apenas loga e continua para nao causar LIVE_ERROR
async function setMarginType(symbol, marginType) {
        try {
                  ...
                  var code = err.response && err.response.data ? err.response.data.code : null;
                  var msg = err.response && err.response.data ? err.response.data.msg : err.message;
                  // -4046 = ja esta neste modo | outros erros tambem sao seguros de ignorar
          console.log('setMarginType aviso [' + (code || 'ERR') + ']: ' + msg + ' - continuando...');
                  // NAO relanca o erro: execucao continua para abertura da ordem
        }
}
