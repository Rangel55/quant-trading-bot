...
async function getWalletBalance() {
    const data = await privateGet('/v5/account/wallet-balance', { accountType: 'UNIFIED', coin: 'USDT' });
...
// Abre posicao LONG a mercado nos Futuros com SL/TP e Trailing Stop opcional
async function openLong(symbol, quantity, entryPrice) {
  ...
  // Trailing Stop: usa activePrice como entryPrice e trailingStop como distancia em valor absoluto
  if (cfg.TRAILING_STOP && entryPrice) {
        var trailDist = (entryPrice * cfg.TRAILING_STOP_PERCENT).toFixed(4);
        params.trailingStop  = trailDist;
        params.activePrice   = entryPrice.toFixed(4);
        console.log('Trailing Stop ativado: distancia $' + trailDist + ' a partir de $' + entryPrice);
        delete params.stopLoss; // Trailing substitui o SL fixo
  }
...
// Abre posicao SHORT a mercado nos Futuros com SL/TP e Trailing Stop opcional
...
async function cancelAllOrders(symbol) {
    symbol = symbol || cfg.SYMBOL;
    try {
          await privatePost('/v5/order/cancel-all', { category: 'linear', symbol: symbol });
...
async function getPositions(symbol) {
    symbol = symbol || cfg.SYMBOL;
    const data = await privateGet('/v5/position/list', { category: 'linear', symbol: symbol });
    return data.result.list || [];
}

module.exports = { getWalletBalance, setLeverage, openLong, openShort, closePosition, cancelAllOrders, getPositions };
